...
export const SITE_SETTINGS_FALLBACK = {
  ...
  announcementText: [
  "Absolute free delivery in LHR for orders above Rs. 5000",
  "Working Days: MON – SAT | Working Time: 10:30 — 8:30",
  "From 15th to 19th, no orders will be entertained",
].join(" • "),
...
// The announcement bar holds several sentences in one CMS field, separated by
// "•" — "|" is already used inside the opening-hours sentence.
...
export function normalizeSiteSettings(raw) {
    ...
    return {
      ...
      announcementItems: splitAnnouncement(
        SITE_SETTINGS_FALLBACK.announcementText
      ),
    };
  }
  ...
  return {
    ...
    announcementItems: splitAnnouncement(
      pick(raw.announcementText, "announcementText")
    ),
